    ...
    if os.path.exists(f):
        try:
            ...
            if (img.width, img.height) not in valid_sizes:
                 print(f"  [ERROR] INVALID SIZE! Expected 1280x800 or 640x400.")
            else:
                 print(f"  [OK] Size OK.")

            if img.mode == 'RGBA' or 'A' in img.mode:
                print(f"  [ERROR] HAS ALPHA CHANNEL (Transparency)! Store requires 24-bit (RGB).")
            else:
                print(f"  [OK] Mode OK (No Alpha).")
            print("-" * 20)
