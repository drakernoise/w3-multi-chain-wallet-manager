...
export const ChatView: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    // Listen for chat-unread events from chatService (when UI receives messages directly via WebSocket)
    useEffect(() => {
        const handleChatUnread = (e: any) => {
                    ...
                    {searchResults.length > 0 && (
                        <div className="absolute left-2 right-2 mt-1 z-50 bg-dark-800 border border-dark-600 rounded-xl shadow-2xl overflow-hidden animate-slideDown">
                            <div className="p-2 border-b border-dark-700 flex justify-between items-center bg-dark-900/50">
                                ...
                                <button onClick={() => setSearchResults([])} className="text-slate-500 hover:text-white">X</button>
                            </div>
                        ...
                        <>
                            ...
                            <div className="h-14 border-b border-dark-700 flex items-center px-4 bg-dark-800 justify-between shrink-0 shadow-md">
                                ...
                                <div className="flex gap-1.5 items-center shrink-0 ml-2">
                                    ...
                                    <button onClick={onClose} className="text-slate-400 hover:text-white hidden md:block p-1.5 transition-colors">X</button>
                                </div>
                            </div>
                            ...
                            <div className="p-2 md:p-3 bg-dark-800 border-t border-dark-700 relative">
                                {/* Emoji picker removed per user request */}
                                <div className="flex items-center gap-2 bg-dark-900 border border-dark-600 rounded-xl px-2 py-1.5 focus-within:ring-2 focus-within:ring-purple-500/50 transition-all">
                                    <div className="p-1.5 text-slate-500">
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8S21 7.582 21 12z" /></svg>
                                    </div>
                                    <input
            ...
            {activeRoomId && showParticipants && (
                <div className="absolute right-0 top-0 bottom-0 w-64 bg-dark-800 border-l border-dark-700 flex flex-col z-50 shadow-2xl">
                    <div className="p-4 border-b border-dark-700 flex justify-between items-center bg-dark-900/50">
                        ...
                        <button onClick={() => setShowParticipants(false)} className="text-slate-500 hover:text-white">X</button>
                    </div>
