...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-16 bg-dark-800 border-r border-dark-700 flex flex-col items-center py-4 gap-6 shrink-0 z-20">
      ...
      <button
        onClick={onLock}
        className="mt-auto mb-2 text-slate-500 hover:text-slate-300 transition-colors p-2"
        title="Lock Wallet"
      >
...
const NavIcon: React.FC<{ active: boolean; onClick: () => void; icon: React.ReactNode; label: string }> = ({ active, onClick, icon, label }) => (
  <button
    onClick={onClick}
    className={`group relative flex items-center justify-center w-10 h-10 rounded-lg transition-all ${active ? 'bg-dark-700 text-blue-400' : 'text-slate-500 hover:bg-dark-700 hover:text-slate-300'}`}
  >
